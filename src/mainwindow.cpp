    ...
    ui(new Ui::MainWindow)
{
    ...
    web_sub = nh.subscribe("/receive_product", 10, &MainWindow::webCallback, this);
...
void MainWindow::webCallback(const std_msgs::Bool::ConstPtr& msg)       //topic /receive_product
{
    if (msg->data) {
        ROS_INFO("Received a True message.");
        QString logMessage = "Received a True message.";
        ui->webtextBrowser->append(logMessage);
    } else {
        ROS_INFO("Received a False message.");
        QString logMessage = "Received a False message.";
        ui->webtextBrowser->append(logMessage);
    }
}
